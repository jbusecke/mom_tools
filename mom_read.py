...
def process_coords(ds, concat_dim='time', drop=True, hard=True,
                   extra_coord_vars=['time_bounds', 'average_T1',
                                     'average_T2', 'average_DT', 'nv']):
...
def clean(ds):
    ...
    remove_attrs = ['time_avg_info', 'cell_methods', 'valid_range', 'coordinates']
    for ra in remove_attrs:
...
def data_vars_consistency(path1, path2, **kwargs):
    ...
    and vice versa"""
    ds1 = xr.open_mfdataset(path1, decode_cf=False, chunks={'time': 1}, **kwargs)
    ds2 = xr.open_mfdataset(path2, decode_cf=False, chunks={'time': 1}, **kwargs)
...
def open_mom5_single_var(ddir, var, years=None, yearfmt='%040101', filenamefmt=None,
    ...
    if drop_inconsistent:
        incon_var = data_vars_consistency(all_fnames[0], all_fnames[-1], **kwargs)
        if incon_var:
            print('%s - Variable inconsistencies between files found for (%s). \
            Variables are dropped. Use `drop_inconsistent=False` to disable.' %(var, incon_var))
        drop_vars += incon_var
    else:
        print('This might take a long time to read the files if incosistencies are present.')
...
def open_mom5_CM_ESM(basedir, timespec='monthly_1yr', subfolder='av', varfolderlist=None, force_varlist=False, 
               reftime=None, years=None, gridfile=None, **kwargs):
    ...
    for va in varfolderlist:
        ...
        if os.path.isdir(path):        
            ...
            if os.path.isdir(os.path.join(basedir, va, subfolder)):
                ...
                print('something is wrong with the path for %s (%s)' %(va, path))
    ...
    if reftime is not None:
        if not all(len(x.time)==len(datasets[0].time) for x in datasets):
            raise RuntimeError('Reference Time replacement can only be done when all datasets\
            have the same length. Current lengths: %s' %[len(d.time) for d in datasets])
